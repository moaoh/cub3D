...
/*   Updated: 2021/01/25 08:37:12 by junmkang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_img_change(t_ray_info *ray_info, t_loop_info *info, t_img *img, int start, int end)
{
	...
	while (temp <= end)
	{
		...
		calc.tex_point = calc.img_y * (img->size_l / 4) + calc.img_x;
		calc.color = img->data[calc.img_y * (img->size_l / 4) + calc.img_x];
		
		// printf("%lf\n", (img->width * (calc.obj_x - floor(calc.obj_x))));
		if (start == 0) calc.color = 0x1DA06A;
		if (start == info->drawEnd) calc.color = 0xFFFFFF;
