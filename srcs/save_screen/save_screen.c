...
/*   Updated: 2021/02/01 00:44:22 by junmkang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		save_screen(t_ray_info *ray_info, t_loop_info *info)
{
	...
	if (!(fd = open("cub3d_save.bmp", O_RDWR)))
		ft_error("cub3d_save.bmp does not exist.");
	write(fd, "BM", 2);
	...
	j = ray_info->screen_Y;
	while (--j >= 0)
		write(fd, ray_info->img[0].data + (ray_info->img[0].size_l / 4) * j, (bitmap_sizeline));
	close(fd);
	exit(0);
}